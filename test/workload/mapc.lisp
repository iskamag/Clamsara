;;;; Managed MAPC behavior. Original independent v2 cases are preserved below.
;;;; Caller-owned failed worlds are retained; cleanup is success-only.
(defpackage #:clamsara.workload.mapc.test
  (:use #:cl #:clamsara)
  (:export #:run-mapc-strict-case #:*owners* #:*cases*))
(in-package #:clamsara.workload.mapc.test)
...
(defvar *mapc-test-runtimes* nil)
(defun run-workload-mapc-tests ()
  (let ((passed 0) (failed 0) (before *owners*))
    (assert (= 9 (length *cases*)))
    (dolist (case *cases*)
      (handler-case
          (let ((runtime (make-workload-runtime :extent 16384 :max-object-bytes 8192
                                                :root-capacity 512)))
            (push (list case runtime) *mapc-test-runtimes*)
            (run-mapc-strict-case
             runtime case :pathname (asdf:system-relative-pathname
                                     :clamsara "bench/gabriel/reference/dderiv.cl"))
            (assert (null (clamsara::workload-runtime-configuration runtime)))
            (incf passed))
        (error (condition)
          (incf failed)
          (format t "~&WORKLOAD-MAPC-FAIL ~S [~S] ~A~%" case (type-of condition) condition))))
    (format t "~&WORKLOAD-MAPC groups=9 passed=~D failed=~D owners=~D~%"
            passed failed (length (ldiff *owners* before)))
    (assert (zerop failed))
    (assert (= 9 passed (length (ldiff *owners* before)))))
  t)
(export 'run-workload-mapc-tests)
