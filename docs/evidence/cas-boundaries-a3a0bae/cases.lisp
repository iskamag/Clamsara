...
(defun event (rule phase operation)
  ;; Hosted test transcript, not production/target storage or a reservation.
  ;; Never retain locations or managed references in this transcript.
  (push (list phase (rule-name rule) operation) *trace*))
...
(defun names-at (phase)
  (loop for event in (reverse *trace*) when (eq (first event) phase)
...
(defun one (label rules body)
  (incf *cases*)
  (let ((*rules* rules) (*trace* nil) (w nil))
    (handler-case
        (progn
      ...
      (error (c)
        (incf *failures*) (push (list label w c (reverse *trace*)) *failed-worlds*)
        (format t "~&CAS-FAIL ~S ~A~%TRACE ~S~%" label c (reverse *trace*))))))
