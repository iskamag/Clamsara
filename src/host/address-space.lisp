...
(defmethod validate-managed-layout
  ...
  (let* ((solutions (%layout-ordered-solutions layout))
    ...
    (dotimes (i (length solutions))
      (let ((s (aref solutions i)))
        (unless (<= base (%placement-solution-base s)
                    (%placement-solution-exclusive-limit s) limit)
          (error 'construction-rejected :reason :layout-outside-arena))
        (when (%simulator-solution-space s)
          (dotimes (j i)
            (let ((other (aref solutions j)))
              (when (and (%simulator-solution-space other)
                         (< (%placement-solution-base s) (%placement-solution-exclusive-limit other))
                         (< (%placement-solution-base other) (%placement-solution-exclusive-limit s)))
