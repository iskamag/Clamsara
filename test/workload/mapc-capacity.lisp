;;;; Physical native MAPC capacity, ownership and protected snapshot witnesses.
;;;; Independent v3 bodies; temporary fixture methods never replace primaries.
(defpackage #:clamsara.workload.mapc-capacity.test
  (:use #:cl #:clamsara)
...
(in-package #:clamsara.workload.mapc-capacity.test)
...
(defun run-workload-mapc-capacity-tests ()
  (let ((passed 0) (failed 0) (before *cap-owners*))
    (cap-observer-self-check)
    (dolist (entry (list (cons :boundaries #'cap-run-boundary-matrix)
                        (cons :snapshot #'cap-run-snapshot-witness)
                        (cons :nested #'cap-run-nested-rejection)
                        (cons :smaller-recovery #'cap-run-smaller-nonempty-recovery)
                        (cons :post-outer-recovery #'cap-run-post-outer-nonempty-recovery)))
      (handler-case (progn (funcall (cdr entry)) (incf passed))
        (error (condition)
          (incf failed)
          (format t "~&WORKLOAD-MAPC-CAPACITY-FAIL ~S [~S] ~A~%"
                  (car entry) (type-of condition) condition))))
    (let ((owners (ldiff *cap-owners* before)))
      (format t "~&WORKLOAD-MAPC-CAPACITY groups=5 passed=~D failed=~D owners=~D~%"
              passed failed (length owners))
      (assert (zerop failed))
      (assert (= passed 5))
      (assert (= (length owners) 17))
      (dolist (owner owners)
        (assert (eq :complete (cap-owner-phase owner)))
        (assert (null (cap-owner-condition owner)))
        (assert (null (clamsara::workload-runtime-configuration
                       (cap-owner-runtime owner)))))))
  t)
(export 'run-workload-mapc-capacity-tests)
