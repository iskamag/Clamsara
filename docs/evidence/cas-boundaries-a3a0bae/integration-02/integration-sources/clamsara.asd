...
(asdf:defsystem :clamsara/test
  ...
  :in-order-to ((asdf:test-op (asdf:test-op :clamsara/protocol/test)
                             ...
                             (asdf:test-op :clamsara/quality/indexed-snapshots/test)
...
(asdf:defsystem :clamsara/quality/indexed-snapshots/test
  :version "0.1.0"
  :description "Hosted indexed identities, staged keys and structural name snapshots."
  :depends-on (:clamsara/quality/kind-snapshots/test)
  :serial t
  :components ((:file "test/quality/indexed-snapshots")
               (:file "test/quality/indexed-history")
               (:file "test/quality/indexed-edges")
               (:file "test/quality/staged-history"))
  :perform (asdf:test-op (operation component)
             (declare (ignore operation component))
             (unless (and (uiop:symbol-call :clamsara.quality.indexed-snapshots
                                           :run-indexed-snapshot-tests)
                          (uiop:symbol-call :clamsara.quality.indexed-history
                                           :run-indexed-history-tests)
                          (uiop:symbol-call :clamsara.quality.indexed-edges
                                           :run-indexed-edge-tests)
                          (uiop:symbol-call :clamsara.quality.staged-history
                                           :run-staged-history-tests))
               (error "Indexed/name snapshot tests failed"))))
