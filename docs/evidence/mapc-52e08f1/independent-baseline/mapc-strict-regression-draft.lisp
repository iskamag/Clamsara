;;;; NEW SOURCE-ONLY DRAFT. Not loaded or executed. No production implementation.
...
(defun phase (owner phase) (setf (owner-phase owner) phase))
...
(defun finish-success (owner)
  ...
  (phase owner :consume-proven-result)
  (workload-eval (env owner) nil)
  (phase owner :discharge)
...
(defun check-direct-empty (owner)
  (phase owner :direct-empty-arity)
...
(defun check-semantics (owner)
  ...
  (dolist (lists '(((1 2))
                   ...
                   ((1 2) nil (5 6))
                   ((1 2) (3 4) nil)))
    (phase owner (list :semantic lists))
...
(defun check-single-result (owner)
  (phase owner :single-mapc-result)
...
(defun check-designator (owner)
  ...
  (phase owner :release-case-owned-definition)
...
(defun check-retry (owner before)
  (phase owner :normal-call-after-nlx)
...
(defun check-error (owner)
  (let ((before (registers owner)))
    (phase owner :expected-callback-error)
    ;; Only this exact intended condition is caught. Unexpected errors escape
    ;; to the retaining owner handler; no EVAL NIL/close happens on that path.
    (assert (handler-case (progn (workload-eval (env owner) *error-form*) nil)
              (expected-mapc-stop () t)))
    (automatic-movement owner)
    (assert (equal before (registers owner)))
    (no-active-scopes owner)
...
(defun check-dderiv-load (owner pathname)
  ...
  (let* ((package (find-package :clamsara))
    ...
    (phase owner :fixture-ownership-precheck)
    (dolist (name names)
    ...
    (dolist (op ops)
      (assert (not (nth-value 1 (gethash (clamsara::%property-key op indicator)
                                        properties)))))
    (phase owner :original-dderiv-load)
    (workload-load (env owner) pathname) ; ALL original forms, unchanged bytes
    (phase owner :four-property-identities)
    ...
    (loop for op in ops for handler in handlers do
      (assert (eq (clamsara::%property-value (client owner) op indicator)
                  (clostrum:fdefinition (client owner) (guest-environment owner)
                                       handler))))
    (phase owner :release-successful-fixture-load-owners)
