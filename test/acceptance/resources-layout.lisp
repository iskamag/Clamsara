...
(defun test-real-layout-and-bound-model-manifest-is-complete ()
  ...
  (let* ((client (clamsara::make-simulator-address-space
         ...
         (offered (clamsara::make-host-object-model
                   :capacity 256 :max-object-bytes 32 :variant-capacity 256
                 ...
                 (labels ((remember (object)
                   ...
                   (dolist (object
                             (list (clamsara::host-model-arena bound)
                                   ...
                                   (clamsara::host-model-variant-code-keys bound)
                                   (clamsara::host-model-variant-code-rows bound)))
                     (remember object))
...
(defun run-resources-layout-acceptance ()
  (let ((failures nil))
    (setf failures (call-test :acquisition-failure
          ...
          failures (call-test :ownership-granularity
                              #'test-ownership-update-keeps-descriptor-granularity failures))
    (when failures
