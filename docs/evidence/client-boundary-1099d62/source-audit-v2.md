# Client boundary audit of committed 1099 — source only, revision 2
...
### D. High accepted-runtime correctness gap: finalizer admission/roots/publication
...
A native closure capturing an otherwise unrooted guest reference passes this
callback type gate; registration does not first reject the unsupported capture
or establish its corrected root ownership. Likewise these accepted registration
writes bypass the composed root-store path required by the paper. These are
missing correctness checks/publication behavior in the accepted runtime path,
not merely an inability to swap in a new opaque backend.
...
future negative witness; no such run was performed here. The missing checks and
root/barrier routes are source-proved; actual lost-capture execution has not been
newly reproduced by this audit.

The source itself says “managed-callback admission remains open” at line 1.
That disclosure prevents a full-conformance claim; it does not turn acceptance
of an unsupported callback into safe admission. **The solution is not to teach
the core to decode Maclina closures.** Admission/invocation/root ownership must
be justified by the client integration and existing contracts, or the
unsupported composition must be rejected before publication.
...
## Priority: correctness first, generalization second

1. **Accepted-runtime correctness (D):** establish or reject callback admission,
   writable callback/capture ownership and required root/barrier publication.
   These missing protections affect an input category the registry currently
   accepts; a hosted-only profile label does not discharge them.
2. **Hosted-only integration/generalization limits (A–C):** justify each private
   host bridge and its exact admitted scope under existing interfaces/ownership.
   They are source-proved backend dependencies, not new native corruption results
   in configurations using the intended hosted model and root locations.
3. **Do not “fix” the counterexamples:** valid declared vector representations,
   client-owned guest ABI and workload-local VM/root decoding are not themselves
   universal-contract violations. Held descriptor-domain changes remain separate.

The requirements below are requirements only. This audit does not choose new
public generic signatures or authorize changes to the paper.
