;;;; test/test-protocols.lisp -- paper-v11 protocol system contracts.
;;;;
...
;;;;      not just claimed (ADAPTERS-NO-ALLOCATION).

(in-package #:clamsara)

(defparameter *protocol-systems*
...
(defun %check (ok ok-msg fail-msg)
  (if ok (values t ok-msg) (values nil fail-msg)))

(deftest protocol-systems-independent ()
  (let ((problems nil))
    ;; structural: findable, and no ASDF dependencies at all
    (dolist (name *protocol-systems*)
      (handler-case
    ...
    (unless problems
      (let* ((asd (asdf:system-source-file :clamsara/test)))
        ...
        (unless (and asd (uiop:file-exists-p asd))
          (return-from protocol-systems-independent
    ...
    (let ((vm *clamsara-vm*))
      (let ((a (clamsara-allocate-object 2)))
        ...
        (unless (and (valid-reference-p vm a) (object-start-p vm a))
          (return-from object-model-adapter (values nil "allocated object not valid/start")))
        (when (object-start-p vm (+ a 1))
          (return-from object-model-adapter (values nil "interior word reported as object start")))
        (when (or (valid-reference-p vm 0) (valid-reference-p vm (vm-heap-size vm)))
          (return-from object-model-adapter (values nil "null/out-of-heap reported valid")))
        (unless (= (object-size vm a) (* 3 +word-bytes+))
          (return-from object-model-adapter (values nil "object-size is not bytes of header+payload")))
        (unless (eql (object-kind vm a) +tag-object+)
          (return-from object-model-adapter (values nil "object-kind mismatch")))
        ...
        (let* ((b (clamsara-allocate-object 3 :layout-id 7))
          ...
          (unless (and (= (length locs) 1) (= (first locs) (+ b 1 1)))
            (return-from object-model-adapter (values nil "map-reference-locations did not name slot word b+2")))
          ...
          (let ((c (clamsara-allocate-object 1)))
            ...
            (unless (eql (load-reference vm (first locs)) c)
              (return-from object-model-adapter (values nil "load-reference mismatch after store")))
            (unless (eql (vm-object-reference vm b 1) c)
              (return-from object-model-adapter (values nil "raw store did not land in the declared slot word")))
            ;; re-mapping yields the same location for the same slot
            (let ((again (let ((found nil))
                           (map-reference-locations vm b (lambda (loc) (push loc found)))
                           (first found))))
              (unless (eql again (first locs))
                (return-from object-model-adapter (values nil "location is not stable across map calls")))))
          ...
          (let ((w (clamsara-allocate-object 1)))
            ...
            (let ((n 0))
              ...
              (unless (zerop n)
                (return-from object-model-adapter (values nil "weak referent slot visited as strong"))))))
        ...
        (let ((d (clamsara-allocate-object 4)))
          ...
          (unless (and (eql (vm-object-type-tag vm d) +tag-struct+)
            ...
            (return-from object-model-adapter (values nil "initialize-object wrote the wrong shape")))
          (unless (= (object-size vm d) 24)
            (return-from object-model-adapter (values nil "object-size disagrees with initialize-object")))
          (unless (handler-case (progn (initialize-object vm d +tag-struct+ 25 0) nil)
                     (clamsara-error () t))
            (return-from object-model-adapter (values nil "non-word size accepted")))
          (unless (handler-case (progn (initialize-object vm d +tag-cons+ 16 0) nil)
                     (clamsara-error () t))
            (return-from object-model-adapter (values nil "cons accepted by initialize-object"))))
        ...
        (let ((src (clamsara-allocate-object 2))
          ...
          (unless (and (vm-object-is-marked-p vm src)
            ...
            (return-from object-model-adapter (values nil "copy transferred logical mark metadata")))
          ...
          (unless (and (eql (vm-object-reference vm dst 1) 12345)
            ...
            (return-from object-model-adapter (values nil "copy did not carry payload"))))
        ...
        (let ((e (clamsara-allocate-object 1)))
          (unless (reference-equal vm e (ref-set-colour vm e 3))
            (return-from object-model-adapter (values nil "coloured reference not identity-equal")))
          (unless (null (offered-metadata-fields vm))
            (return-from object-model-adapter (values nil "simulator advertised metadata fields")))
          (unless (handler-case (progn (field-read vm :mark e) nil)
                     (clamsara-error () t))
            (return-from object-model-adapter (values nil "field-read accepted on a fieldless client"))))
        (values t "object-model adapter ok")))))

(deftest root-protocol-adapter ()
    ...
    (let ((vm *clamsara-vm*))
      (let* ((o1 (clamsara-allocate-object 1))
        ...
        (unless (and (= i1 0) (= i2 1))
          (return-from root-protocol-adapter (values nil "root indices unexpected")))
        ...
        (unless (and (= (count :root-vector kinds) 2)
          ...
          (return-from root-protocol-adapter (values nil "snapshot missed root vector cells")))
        ...
        (let ((loc2 (let ((found nil))
          ...
          (unless (eq (root-location-kind vm loc2) :root-vector)
            (return-from root-protocol-adapter (values nil "o2 location is not a root-vector cell")))
          (store-root vm loc2 0)
          (unless (eql (clamsara-root i2) 0)
            (return-from root-protocol-adapter (values nil "store-root did not write the root vector"))))
        ...
        (let ((region-loads nil) (region-loc nil))
          ...
          (unless (= (length region-loads) 2)
            (return-from root-protocol-adapter (values nil "region cells not visited")))
          ;; REGION-LOC is the last visited region cell (index 1)
          (store-root vm region-loc 77)
          (unless (= (aref regvec 1) 77)
            (return-from root-protocol-adapter (values nil "store-root did not write the region vector"))))
        ;; honest boundaries: unsupported scope, unknown location
        (unless (handler-case (progn (with-root-snapshot vm :request #'identity) nil)
                   (clamsara-error () t))
          (return-from root-protocol-adapter (values nil "request scope accepted")))
        (unless (handler-case (progn (load-root vm 1234567890123) nil)
                   (clamsara-error () t))
          (return-from root-protocol-adapter (values nil "bogus root location accepted")))
        (values t "root adapter ok")))))

(deftest coordination-adapter ()
    ...
    (let ((vm *clamsara-vm*))
      ...
      (let ((token (request-safepoint vm :all :test)))
        (unless (vm-mutators-stopped-p vm)
          (return-from coordination-adapter (values nil "request-safepoint did not stop the mutators")))
        (unless (= (await-safepoint vm token) (vm-safepoint-epoch vm))
          (return-from coordination-adapter (values nil "await-safepoint returned a foreign epoch")))
        ...
        (let ((e (vm-safepoint-epoch vm)))
          ...
          (unless (= (vm-safepoint-epoch vm) e)
            (return-from coordination-adapter (values nil "repeat request re-numbered the active stop"))))
        (release-safepoint vm token)
        (when (vm-mutators-stopped-p vm)
          (return-from coordination-adapter (values nil "release-safepoint left mutators stopped")))
        ;; a stale token after release rejects on await AND release
        (unless (handler-case (progn (await-safepoint vm token) nil)
                   (clamsara-error () t))
          (return-from coordination-adapter (values nil "stale await accepted")))
        (unless (handler-case (progn (release-safepoint vm token) nil)
                   (clamsara-error () t))
          (return-from coordination-adapter (values nil "stale release accepted")))
        (unless (handler-case (progn (await-safepoint vm :not-a-token) nil)
                   (clamsara-error () t))
          (return-from coordination-adapter (values nil "foreign safepoint token accepted"))))
      (unless (eq (current-mutator vm) :single-mutator)
        (return-from coordination-adapter (values nil "current-mutator mismatch")))
      ...
      (let ((e (begin-epoch vm :default)))
        (unless (eq e (vm-coordination-state vm))
          (return-from coordination-adapter (values nil "epoch token is not the client's coordination state")))
        (unless (plusp (await-epoch vm e))
          (return-from coordination-adapter (values nil "await-epoch returned no epoch")))
        (unless (handler-case (progn (await-epoch vm e) nil)
                   (clamsara-error () t))
          (return-from coordination-adapter (values nil "stale second await accepted")))
        (let ((e2 (begin-epoch vm :default)))
          (unless (eq e2 e)
            (return-from coordination-adapter (values nil "token did not identify the same client")))
          (unless (handler-case (progn (begin-epoch vm :default) nil)
                     (clamsara-error () t))
            (return-from coordination-adapter (values nil "overlapping begin-epoch accepted")))
          (unless (plusp (await-epoch vm e2))
            (return-from coordination-adapter (values nil "second epoch did not close cleanly")))))
      (unless (handler-case (progn (await-epoch vm :other-client-token) nil)
                 (clamsara-error () t))
        (return-from coordination-adapter (values nil "foreign epoch token accepted")))
      (unless (handler-case (progn (begin-epoch vm :other-domain) nil)
                 (clamsara-error () t))
        (return-from coordination-adapter (values nil "undeclared epoch domain accepted")))
      (publish-fence vm)
      (values t "coordination adapter ok"))))

(deftest atomics-adapter ()
  (with-clamsara (:plan-type :nogc :heap-size 4096)
    (let ((vm *clamsara-vm*)
          (place 4090))
      (atomic-store vm place 42 :seq-cst)
      (unless (= (atomic-load vm place :seq-cst) 42)
        (return-from atomics-adapter (values nil "atomic store/load mismatch")))
      (unless (= (atomic-load vm place :acquire) 42)
        (return-from atomics-adapter (values nil "acquire load rejected")))
      (let ((prev (atomic-cas vm place 42 100 :seq-cst)))
        (unless (and (= prev 42) (= (atomic-load vm place :relaxed) 100))
          (return-from atomics-adapter (values nil "successful cas reported wrong previous")))
        (let ((prev2 (atomic-cas vm place 7 8 :seq-cst)))
          (unless (and (= prev2 100) (= (atomic-load vm place :relaxed) 100))
            (return-from atomics-adapter (values nil "failing cas mutated the place")))))
      (unless (= (atomic-fetch-add vm place 5 :release) 100)
        (return-from atomics-adapter (values nil "fetch-add did not return previous")))
      (unless (= (atomic-load vm place :relaxed) 105)
        (return-from atomics-adapter (values nil "fetch-add lost the delta")))
      ...
      (unless (eq (atomic-bit-set vm place 4 :seq-cst) nil)
        (return-from atomics-adapter (values nil "bit-set on a clear bit returned true")))
      (unless (= (atomic-load vm place :relaxed) 121)
        (return-from atomics-adapter (values nil "bit-set did not set bit 4")))
      (unless (eq (atomic-bit-set vm place 4 :seq-cst) t)
        (return-from atomics-adapter (values nil "bit-set on a set bit returned false")))
      (unless (eq (atomic-bit-clear vm place 4 :seq-cst) t)
        (return-from atomics-adapter (values nil "bit-clear on a set bit returned false")))
      (unless (eq (atomic-bit-clear vm place 4 :seq-cst) nil)
        (return-from atomics-adapter (values nil "bit-clear on a clear bit returned true")))
      (fence vm :seq-cst)
      (fence vm :acquire)
      ;; order validation: nonsense, weaker, and unknown orders reject
      (unless (handler-case (progn (atomic-load vm place :release) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "load accepted a release order")))
      (unless (handler-case (progn (atomic-store vm place 1 :acquire) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "store accepted an acquire order")))
      (unless (handler-case (progn (atomic-load vm place nil) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "load accepted a NIL order")))
      (unless (handler-case (progn (atomic-load vm place :bogus) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "load accepted an unknown order")))
      (unless (handler-case (progn (fence vm :relaxed) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "relaxed fence accepted")))
      ;; bounds are checked, not silently out-of-heap
      (unless (handler-case (progn (atomic-load vm (vm-heap-size vm) :seq-cst) nil)
                 (clamsara-error () t))
        (return-from atomics-adapter (values nil "out-of-heap atomic place accepted")))
      (values t "atomics adapter ok"))))

(deftest address-space-adapter ()
  ;; The simulator client offers its word-addressed heap as ONE kernel
  ;; arena (page 0 an implementation reservation, trailing partial pages
  ;; excluded), and validate/install consume the kernel's layout solution.
  (with-clamsara (:plan-type :nogc :heap-size 8192)
    (let* ((vm *clamsara-vm*)
           (plan *clamsara-plan*)
           (layout (vm-managed-layout vm)))
      (let ((offer (managed-arena-offer vm)))
        (unless (and (= (length (offer-arenas offer)) 1)
                     (eq (arena-name (first (offer-arenas offer)))
                         :simulator-heap)
                     (= (arena-base (first (offer-arenas offer))) 0)
                     (= (arena-extent (first (offer-arenas offer)))
                        (vm-heap-size vm))
                     (= (arena-page-size (first (offer-arenas offer)))
                        +page-words+)
                     (equal (arena-access-modes (first (offer-arenas offer)))
                            '(:read :write))
                     ;; the null sentinel is a reservation, never an exclusion
                     (and (= 1 (length (arena-reservations
                                        (first (offer-arenas offer)))))
                          (zerop (reservation-start
                                  (first (arena-reservations
                                          (first (offer-arenas offer))))))))
          (return-from address-space-adapter
            (values nil "arena offer is not the simulator heap"))))
      ;; the booted plan installed a complete, validated solution
      (unless (and (typep layout 'managed-layout)
                   (solution-installed-p layout)
                   (eq layout (install-managed-layout vm layout)))
        (return-from address-space-adapter
          (values nil "the booted plan did not install its layout solution")))
      ;; every region lies inside the arena, page-aligned, non-overlapping
      (let ((end-so-far 0))
        (dolist (region (solution-regions layout))
          (unless (and (zerop (mod (region-start region) +page-words+))
                       (plusp (region-extent region))
                       (>= (region-start region) end-so-far)
                       (<= (region-end region) (vm-heap-size vm)))
            (return-from address-space-adapter
              (values nil (format nil "region ~a escapes the arena"
                                  (region-name region)))))
          (setf end-so-far (region-end region))))
      ;; space resolution derives from the installed solution's geometry
      (let* ((o (clamsara-allocate-object 1))
             (space (space-of-reference plan o)))
        (unless (and (typep space 'space) (eq space (default-space plan)))
          (return-from address-space-adapter
            (values nil "space-of-reference did not resolve")))
        (unless (null (space-of-reference plan (vm-heap-size vm)))
          (return-from address-space-adapter
            (values nil "unmanaged address resolved to a space"))))
      ;; ownership reassignment belongs to the layout, under its epoch
      ;; discipline: an update without a quiesced epoch token is refused
      (unless (handler-case
                  (progn (update-space-ownership
                          plan (cons +page-words+ +page-words+)
                          (region-name (first (solution-regions layout))))
                         nil)
                (layout-rejection () t))
        (return-from address-space-adapter
          (values nil "ownership update accepted without a quiesced epoch")))
      (let ((token (open-ownership-epoch layout)))
        (quiesce-ownership-epoch layout token)
        (unless (eq (region-name
                     (update-space-ownership
                      plan (cons +page-words+ +page-words+)
                      (region-name (first (solution-regions layout)))))
                    (region-name (first (solution-regions layout))))
          (return-from address-space-adapter
            (values nil "quiesced epoch did not authorize the update"))))
      (values t "address-space adapter ok"))))

(deftest mapping-adapter ()
  (with-clamsara (:plan-type :nogc :heap-size 8192)
    (let ((vm *clamsara-vm*))
      ...
      (unless (and (= (car (aref (mmu-vpt vm) 20)) 0)
        ...
        (return-from mapping-adapter (values nil "reserve did not leave reserved pages unmapped/protected")))
      (map-logical-pages vm (cons 20 2) 3 :read-write)
      (unless (and (= (vm-page-physical vm 20) 3) (= (vm-page-physical vm 21) 4))
        (return-from mapping-adapter (values nil "map-logical-pages did not map the source pages")))
      (map-logical-pages vm (cons 20 1) 5 :read)
      (unless (and (= (vm-page-physical vm 20) 5)
                   (eq (cdr (aref (mmu-vpt vm) 20)) :read))
        (return-from mapping-adapter (values nil "map-logical-pages ignored ACCESS")))
      (remap-logical-pages vm 5 21 1)
      (unless (= (vm-page-physical vm 21) 5)
        (return-from mapping-adapter (values nil "remap-logical-pages did not re-back the destination")))
      (protect-logical-pages vm (cons 20 2) :none)
      (unless (eq (cdr (aref (mmu-vpt vm) 21)) :none)
        (return-from mapping-adapter (values nil "protect-logical-pages ignored")))
      (unless (null (flush-address-translations vm (cons 20 2)))
        (return-from mapping-adapter (values nil "flush returned asynchronously for the software MMU")))
      (unmap-logical-pages vm (cons 20 2))
      (unless (and (= (car (aref (mmu-vpt vm) 20)) 0)
                   (eq (cdr (aref (mmu-vpt vm) 20)) :none))
        (return-from mapping-adapter (values nil "unmap-logical-pages left backing")))
      (values t "mapping adapter ok"))))

(deftest diagnostics-adapter ()
  ;; NOTE: fatal-diagnostic is a recorded conformance-blocker (see
  ;; IMPLEMENTATION.md): the portable implementation is allocating.  This
    ...
    (let ((vm *clamsara-vm*))
      (let ((t1 (monotonic-clock vm)))
        (unless (and (typep t1 'fixnum) (>= t1 0) (>= (monotonic-clock vm) t1))
          (return-from diagnostics-adapter (values nil "monotonic clock is not monotonic"))))
      (unless (handler-case (progn (fatal-diagnostic vm :corrupt-heap) nil)
                 (clamsara-error () t))
        (return-from diagnostics-adapter (values nil "fatal-diagnostic did not report")))
...
#+sbcl
(deftest adapters-no-allocation ()
    ...
    (let ((vm *clamsara-vm*))
      ...
      (let* ((o (clamsara-allocate-object 100 :layout-id 11))
        ...
        (unless (vm-valid-reference-p vm o)
          (return-from adapters-no-allocation (values nil "warm object missing")))
            ...
            (loop
              ...
              (let ((before %bytes-allocated))
                (dotimes (k 100)
                  (map-reference-locations vm o ref-visit)
                  (with-root-snapshot vm :all snap-visit))
                (setf bytes (- %bytes-allocated before)))
              (when (or (zerop bytes) (>= attempts 3))
                (return)))
            (%check (and bytes (zerop bytes))
