;;;; Hosted opaque-admission edges: four real histories and sixteen metadata
;;;; capability checks. The latter are not execution/domain/target admission.
;;;; Based on the unchanged independent edge suite; no load-time test runner.
(defpackage #:clamsara.quality.barrier-admission-edges
  (:use #:cl #:clamsara #:clamsara.quality.support)
  (:import-from #:clamsara.quality.barrier-admission
...
(in-package #:clamsara.quality.barrier-admission-edges)
...
(defun compile-other-host-branch ()
  ;; Read the frozen implementation with only :SBCL suppressed, then compile its
  ;; selected body as a NEW anonymous host function. Never DEFUN the public
  ;; helper and never claim this is compilation on another implementation.
  (let ((*features* (remove :sbcl *features*))
        (*package* (find-package :clamsara))
        (*read-eval* nil)
        (eof (gensym "EOF")))
    (with-open-file (stream (asdf:system-relative-pathname :clamsara "src/runtime/barrier.lisp"))
      (let ((form
              (loop for form = (read stream nil eof)
                    until (eq form eof)
                    when (and (consp form) (eq (first form) 'defun)
                              (eq (second form) 'clamsara::%barrier-primary-candidate-p))
                    return form)))
        (check form "No frozen admission helper source found")
        (compile nil (cons 'lambda (cddr form)))))))
...
(defun run-opaque-admission-edge-tests ()
  (let ((*cases* 0) (*failures* 0) (*probe-bodies* 0)
        ...
        (helper (fdefinition 'clamsara::%barrier-primary-candidate-p))
        (actual (make-instance 'probe-actual)))
    (dolist (mode '(nil :retry-transform))
      (clamsara.quality.barrier-admission::one 'inherited-contribution mode))
    ...
    (meta-one :other-host-branch-is-capability-rejection-only
      (lambda ()
        (let ((function (compile-other-host-branch)))
          (signals-runtime-reason (lambda () (funcall function nil nil nil nil))
                                  :unsupported-barrier-method-admission))))
    (meta-one :production-method-combinations-unchanged
      (lambda ()
        (check (eq helper (fdefinition 'clamsara::%barrier-primary-candidate-p))
               "Capability fixture redefined the production admission helper")
    ...
    (check (= *cases* 20) "Opaque edge matrix changed: ~D" *cases*)
