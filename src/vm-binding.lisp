...
;;;
;;; Architecture:
;;;   vm-binding         — abstract protocol class, pure interface (no defaults)
;;;   simulator-vm       — reference implementation using global *HEAP* etc.
;;;   maclina-vm          — inherits simulator-vm, overrides root scanning
;;;
;;; To port Clamsara to a new runtime, subclass vm-binding directly and
;;; implement all generics. See MMTk's VMBinding trait for the same pattern.
...
(defclass vm-binding ()
  ...
  (:documentation "Abstract VM binding protocol.
Provides default method implementations that assume a simulator-like
environment (global *HEAP*, *METADATA-WORDS*, *PAGE-TABLE*).
Both simulator-vm and maclina-vm inherit these defaults.

To port to a real VM (SBCL, CCL, etc.), subclass vm-binding and override
every method that touches heap state. Plans and tracers interact with the
VM exclusively through this protocol."))

;;; --- VM Feature Mixins ---
;;; Generated by defvm-feature from metaclass.lisp; used by vm-has-feature-p.

(defvm-feature :virtual-memory
  :documentation "Hardware-supported dirty page tracking. Enables no-op write barriers
for generational collection when the OS/hardware provides dirty-bit tracking.")

(defvm-feature :has-cas
  :documentation "Hardware CAS support. Required for concurrent GC and lock-free
forwarding pointer updates.")

(defvm-feature :headerless-cons
  :documentation "Headerless cons cells. The VM natively represents cons cells
without a header word, using address alignment to distinguish them.")
...
(defmethod vm-has-feature-p ((vm vm-binding) feature)
  "Check if VM supports FEATURE by looking up the corresponding mixin class.
Returns T if the VM's class inherits from FEATURE-MIXIN."
  (let* ((feature-name (string feature))
         (mixin-name (intern (concatenate 'string feature-name "-MIXIN") #.*package*)))
    (and (find-class mixin-name nil)
         (typep vm mixin-name))))
